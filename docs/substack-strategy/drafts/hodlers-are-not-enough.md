...
The math of who is an economic user, in Adam's sense, is not a math of headcount. It is a math of transaction flow. And by that math, right now, a few dozen institutions are the economic majority, and thirty thousand plebs with cold-stored coins are, in aggregate, background noise.
...
- Node-count figure: the piece states approximately thirty thousand publicly reachable full nodes, per current bitnodes.io / coin.dance snapshots. Verify against latest data before publishing if the count has drifted materially.
