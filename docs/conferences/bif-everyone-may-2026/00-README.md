# Bitcoin is for Everyone — May 22–23, 2026 — Beth Fletcher representing BFTA

Beth Fletcher (BFTA volunteer + featured artist `bitcoinforthearts.org/stories/beth-alta-fletcher`) is representing BFTA at the **Bitcoin is for Everyone** conference in Portland (Friday workshops at the Kennedy School + Saturday main conference at Revolution Hall).

## Why this matters

Three benefits compound at this event:

1. **BFTA brand exposure** to a fresh Bitcoin community Dion has not personally pitched yet
2. **Sponsor leads for Art + Zap Weekend** (September 2026) — captured 4 months before the event
3. **Beth's own artist platform** — direct sales + LN tips at her booth, all 100% hers

This is the right kind of decentralized representation: a real artist showing up where Dion can't be, on behalf of an org she actually believes in.

## What's in this folder

| File | Purpose |
|---|---|
| `00-README.md` | This file — overview |
| `01-reply-to-eric-bif-everyone.md` | **Beth's** reply to Eric Blackstone (continuing her existing thread) confirming the booth + answering his 3 questions, with Dion cc'd. BFTA pays the $300 direct. |
| `02-beth-conference-runner.md` | Beth's tactical playbook for the weekend — talking points, lead capture, what to bring, what to give away |
| `03-beth-coordination-and-compensation.md` | Direct message from Dion to Beth covering: compensation, travel coverage, what BFTA needs from her, what to expect |

## Key decisions baked in (per Dion)

- **BFTA pays the $300 booth fee directly** to Eric. Beth doesn't front it.
- **BFTA covers Beth's travel up to $500** (cap) as a Programs expense.
- **100% of any direct sales** at Beth's booth (her art, her merch, LN tips) are hers.
- **Beth gets a written FMV acknowledgment letter** for her contributed services post-conference.
- **One discount code from Eric** for Beth to share with the BFTA community via newsletter + social.
- **No external Portland artist support** — Beth is solo. The runner accounts for this with simplified booth setup that one person can run.
- **BFTA ships all org materials directly to the conference venue** (per Beth's request) — table banner, T-shirt for Beth, printed one-pagers, QR cards, stickers, sign-up clipboard. Beth carries only her own art + personal items.
- **Beth wears a BFTA T-shirt** during the event (Dion sends in advance after confirming her size via Signal). She can layer or pair with her own swag if she wants — they're complementary, not competing.
- **Inventory on hand:** 3 BFTA T-shirts, 20 stickers. Table banner being ordered this week. **Additional 100 stickers being ordered from StickerMule** (~$50 + $9 ship, ~5-day production) so total = 120 stickers in the box. No standing banner.
- **Shipping timing:** Dion ships the box from NYC the week after Vegas (back May 1) — arrives at the conference venue ~1 week before the May 22 event start.
