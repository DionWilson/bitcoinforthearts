...
export async function POST(
  ...
  if (!emails.length) {
    return NextResponse.json({ ok: false, error: 'Please provide at least one email.' }, { status: 400 });
  }
  const bad = emails.filter((e) => !isEmail(e));
  if (bad.length) {
    return NextResponse.json({ ok: false, error: `Invalid emails: ${bad.join(', ')}` }, { status: 400 });
  }
  ...
  return NextResponse.json({ ok: true, reviewUrl, expiresAt: expiresAt.toISOString() }, { status: 200 });
}
